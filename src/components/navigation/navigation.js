...
const Navigation = () => {
  ...
  return (
    <div className={isMobileNavbar ? `${styles["header"]} ${["mobile--header"]}` : `${styles["header"]}`}>
      <Layout>
        <div className={styles["header--items"]}>
          <Image onClick={() => { setIsMobileNavbar(!isMobileNavbar) }} className={styles["menu-icon"]} src={menuIcon} alt="menu icon" width={32} />
          ...
          <div className={styles["header--items--container"]}>
            <div className={styles["header__account--options"]}>
              ...
              <div  ref={ref} className={styles.dropdown_main} onClick={() => setIsComponentVisible(!isComponentVisible)}>
              ...
              {isComponentVisible && ls ? (<div className={styles.dropdown} onClick={() => {
                    localStorage.removeItem('token');
                    localStorage.removeItem('role');
                    router.push('/login')
                  }}>
                  <p className={styles.dropdown__child} >Logout</p>
                </div>) : null}
              </div>
            </div>
          </div>
